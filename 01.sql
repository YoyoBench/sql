/*
CREATE TABLE [CQC].[dbo].[AdolDep6ClinicScores] (Indexstart Date, IndexEnd Date, Startdate Date, Enddate Date, DateSpan varchar(50), OrgScore DECIMAL(5,2), ComoRoseville DECIMAL(5,2), EastSide DECIMAL(5,2), Highland DECIMAL(5,2), IGH DECIMAL(5,2), NSP DECIMAL(5,2), Shoreview DECIMAL(5,2), Vadnais DECIMAL(5,2),
Banning DECIMAL(5,2), WSP DECIMAL(5,2), Woodbury DECIMAL(5,2))
*/

--Select Indexstart, Indexend, Startdate, CONVERT(varchar,DATEPART(month, Enddate),5)+'/'+CONVERT(varchar,DATEPART(YEAR, Enddate),101) [End],DateSpan,OrgScore,ComoRoseville,EastSide,Highland,IGH,NSP,Shoreview,Vadnais,Banning,WSP,Woodbury from [CQC].[dbo].[AdolDep6ClinicScores] Order By Startdate desc
...
SET @startDate1 = dateadd(m, -12-4, getdate()-day(getdate())+1);
SET @endDate1 = dateadd(m, -4, getdate()-day(getdate()));

--SET @startDate1 = '08-01-2019'
--SET @endDate1 = '07-31-2020'
--Index Date then changes to start 7 months before the reporting period. This is because follow up runs 6 months later + 30 days. 
SET @startDate = DATEADD(MONTH, - 7, @startDate1);
...
INSERT INTO [CQC].[dbo].[AdolDep6ClinicScores] (Indexstart, Indexend, Startdate, Enddate, DateSpan, OrgScore, ComoRoseville, EastSide, Highland, IGH, NSP, Shoreview, Vadnais, Hugo, WSP, Woodbury)
						...
						LEFT JOIN 
							(
								...
								WHERE 
									...
									AND enc.[date] BETWEEN DATEADD(DAY, - 60, DATEADD(MONTH, 6, @startDate))
									AND DATEADD(DAY, 60, DATEADD(MONTH, 6, @endDate))
								...
								ON Follow.patientID = initital.patientID
									AND follow.[date] BETWEEN DATEADD(DAY, - 60, DATEADD(MONTH, 6, initital.[date]))
									AND DATEADD(DAY, 60, DATEADD(MONTH, 6, initital.[date]))
							GROUP BY 
								initital.patientID,
								initital.[date]
					) PHQFollowDate
